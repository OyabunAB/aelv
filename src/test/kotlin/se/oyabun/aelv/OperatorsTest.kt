...
class OperatorsTest {
    ...
    class Expand {
        ...
        fun `flatMap expands each item`() = runTest {
            ...
            assertIs<Success<Set<Int>>>(result)
        ...
        @Test
        fun `concatMap preserves order when all items arrive from upstream at once`() = runTest {
            val result = Many.generate<Int> { emit ->
                listOf(1, 2, 3).forEach { emit(Signal.Upstream.Next(it)) }
                emit(Signal.Upstream.Complete)
            }.concatMap { Many.items(it, it * 10) }.toList().await()
            assertIs<Success<List<Int>>>(result)
            assertEquals(listOf(1, 10, 2, 20, 3, 30), result.value)
        }
    ...
    class GroupBy {
        ...
        fun `groups items by key`() = runTest {
            val result = Many.items(1, 2, 3, 4, 5, 6)
                ...
                .toList()
                .await()
            assertIs<Success<List<Pair<Int, Int>>>>(result)
    ...
    class DistinctUntilChangedBy {
        ...
        fun `distinctUntilChangedBy uses key for comparison`() = runTest {
            ...
            assertIs<Success<List<Item>>>(result)
    ...
    class Interval {
        ...
        fun `interval emits incrementing longs starting at zero`() = runTest {
            ...
            assertIs<Success<List<Long>>>(result)
